...
char
_rl_find_completion_word (size_t* fp, size_t* dp) {
  int found_quote=0, pass_next, isbrk;
  char quote_char = '\0', *brkchars;
  size_t scan, delimiter=0, end;
  ...
  if (scan)
	...
	isbrk = (found_quote == 0 ||
		...
		strchr (brkchars, (int)scan) != 0;
      else
	isbrk = strchr (brkchars, (int)scan) != 0;
      ...
      if (isbrk)
	{
	  ...
	  if (rl_basic_quote_characters &&
	      strchr (rl_basic_quote_characters, (int)scan) &&
	      (end - rl_point) > 1)
	    delimiter = scan;

	  /* If the character isn't needed to determine something special
	     about what kind of completion to perform, then advance past it. */
	  if (rl_special_prefixes == 0 || strchr (rl_special_prefixes, (int)scan) == 0)
	    rl_point++;
	}
    }
     ...
     char **matches;
{
  ...
  if (i)
    qsort (matches+1, i-1, sizeof (char *), (_CoreCrtNonSecureSearchSortCompareFunction)_rl_qsort_string_compare);
     ...
     const char *text;
{
  ...
  if (low == 0 && text && *text)
      ...
      if (_rl_completion_case_fold)
	{
	  ...
	  qsort (match_list+1, matches, sizeof(char *), (_CoreCrtNonSecureSearchSortCompareFunction) _rl_qsort_string_compare);
     ...
     int len, max;
{
  ...
  if (rl_ignore_completion_duplicates == 0)
    qsort (matches + 1, len, sizeof (char *), (_CoreCrtNonSecureSearchSortCompareFunction) _rl_qsort_string_compare);
     ...
     int what_to_do;
{
  char **matches;
  rl_compentry_func_t *our_func;
  int i, nontrivial_lcd;
  char *text, *saved_line_buffer;
  char quote_char;
  size_t start, end, delimiter, found_quote;
     ...
     int state;
{
  ...
  if (GetUserNameA (user_name, &user_len))
    {
      if (namelen == 0 || (!_strnicmp (username, user_name, strlen(user_name))))
	{
...
# define DIR void
  ...
  if (state == 0)
	...
	strcat (tmp, "/*");
      ...
      directory = FindFirstFileA (tmp, &entry);
      found = 1;
#else  
...
#ifdef _WIN32
      found = FindNextFileA (directory, &entry);
#endif    
    }
     ...
     int count, ignore;
{
  ...
  static size_t orig_start, orig_end;
