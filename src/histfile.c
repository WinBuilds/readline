     ...
     int from, to;
{
  ...
  file = _open (input, O_RDONLY|O_BINARY, 0666);
  ...
  chars_read = _read (file, buffer, file_size);
#endif
  if (chars_read < 0)
    {
  error_and_exit:
     ...
     int lines;
{
  ...
  file = _open (filename, O_RDONLY|O_BINARY, 0666);
  rv = 0;
  ...
  /* Write only if there are more lines in the file than we want to
     truncate to. */
  if (bp > buffer && ((file = _open (filename, O_WRONLY|O_TRUNC|O_BINARY, 0600)) != -1))
    {
      _write (file, bp, chars_read - (bp - buffer));
     ...
     int nelements, overwrite;
{
  ...
  if ((file = _open (output, mode, 0600)) == -1)
    {
      FREE (output);
      return (errno);
    }
...
mmap_error:
    ...
    if (msync (buffer, buffer_size, 0) != 0 || munmap (buffer, buffer_size) != 0)
      rv = errno;
#else
    if (_write (file, buffer, buffer_size) < 0)
      rv = errno;
    free (buffer);
#endif
  }
