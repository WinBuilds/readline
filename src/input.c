...
extern size_t rl_point;
extern int rl_done;
extern size_t rl_visible_prompt_length;
      ...
      else
        {
          if (cstat_flags & RLPOS_CHANGED)
		   ...
		   && (mev.dwMousePosition.Y == src_down_pos.Y) )
                {
                  size_t linear_pos = (int)mev.dwMousePosition.Y * _rl_screenwidth + (int)mev.dwMousePosition.X;
                  if (linear_pos < rlScreenStart + rl_visible_prompt_length)
                    {
                      linear_pos = rlScreenStart + rl_visible_prompt_length;
                      mev.dwMousePosition.X = rlScreenOrigin.X + (int)rl_visible_prompt_length;
...
{
  if (_isatty (_fileno (rl_instream)))
    return (_kbhit());
  return 0;
}
...
{
  int count = 0;
  while (_isatty (_fileno (rl_instream)) && _kbhit () && ibuffer_space ())
    {
